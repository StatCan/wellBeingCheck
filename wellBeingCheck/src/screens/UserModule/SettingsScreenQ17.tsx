...
import {View,Text,StyleSheet,ScrollView,Platform,Switch,AsyncStorage,Dimensions,Linking,PanResponder,Alert,BackHandler,AppState,ActivityIndicator,Modal,
} from "react-native";
...
class SettingsScreen extends React.Component<Props, SettingsState> {
  ...
  componentDidMount() {
    ...
    console.log(
      "temps time for sleep time this.state.sleeptime.substring(5,7))",
      this.state.sleeptime.substring(5, 7)
    );
    console.log(
      "temps time for sleep time (parseInt(this.state.sleeptime.substring(0,2)) + 12).toString())",
      (parseInt(this.state.sleeptime.substring(0, 2)) + 12).toString()
    );
    console.log(
      "temps time for sleep time this.state.sleeptime)",
  ...
  async wakeTimeHandler(time) {
    ...
    console.log(
      "wakeup time handler------------------------------------time from time picker before the substring:" +
        time
    );
    //time = time.substring(0, 5);
    console.log(
      "wakeup time handler------------------------------------time from time picker:" +
        time
    );
    ...
    if (resources.culture == "fr") {
      valid = validateSetting(
        time,
        this.state.sleeptime,
        this.state.notificationcount
      );
    } else {
      let w = this.timeTo24(time);
      let s = this.timeTo24(this.state.sleeptime);
    ...
    if (valid != 0) {
      Alert.alert("", resources.getString("settingValidation"));
      return;
    }
  ...
  async sleepTimeHandler(time) {
    ...
    console.log("get the time substring missing the PM " + time);
    console.log(
      "get the time substring missing the PM " + time.substring(0, 9)
    );
    ...
    if (resources.culture == "fr") {
      ...
      let w = this.timeTo24(this.state.waketime);
      let s = this.timeTo24(time);
      valid = validateSetting(w, s, this.state.notificationcount);
    }
  ...
  timeTo24(time) {
    ...
    if ((AMPM == "PM" || AMPM == "P.M." || AMPM == "p.m.") && hours < 12)
      hours = hours + 12;
    if ((AMPM == "AM" || AMPM == "A.M." || AMPM == "a.m.") && hours == 12)
      hours = hours - 12;
        ...
        [
          ...
          },
          {
            ...
            onPress: () => {
              ...
              if (Platform.OS === "ios") {
                Linking.openURL(
                  "app-settings://notification/com.statcan.wellbeingcheck"
                );
              } else {
                if (global.debugMode)
                  console.log("Opening Android Settings Screen");
  ...
  handleBackAction = async (f) => {
    ...
    if (this.state.notificationState) {
      let today = new Date();
      console.log("xxxxxxxxxxxxxxxxxxxx:" + global.warningDate);
      ...
      if (global.notificationState) {
        ...
        global.sendouts = "";
        AsyncStorage.setItem("Sendouts", sendouts);
    ...
    if (dirty || this.state.notificationState != global.notificationState) {
      AsyncStorage.removeItem("ParadataSaved");
  ...
  _handleNotification = (notification) => {
    if (global.debugMode)
      console.log("Notification was clicked - navigating to Survey");
  ...
  _debugClearAllLocalData() {
    try {
      ...
      AsyncStorage.removeItem("hasImage");
  ...
  _storeSettings = (f) => {
    ...
    else AsyncStorage.setItem("NotificationState", "false");
  ...
  _retrieveData = async (key) => {
    await AsyncStorage.getItem(key, (err, result) => {
      ...
      if (result) {
        ...
        this.setState({
          chosenNotificationState: resultAsObj.chosenNotificationState,
        });
    ...
    if (global.debugMode)
      console.log("Resources culture is: " + resources.culture);
  ...
  am_pm_to_hoursSleep(time) {
    console.log(
      "this how to get P from PM" + this.state.sleeptime.substring(5, 7)
    );
    console.log(
      "this how to get hour number + 12 " +
        (parseInt(this.state.sleeptime.substring(0, 2)) + 12)
    );
  ...
  hours_am_pmWake(time) {
    ...
    if (hours < 12) {
      ...
      console.log(
        "here the wake time mins change from 24 to ampm:    " +
          hours +
          ":" +
          min +
          " AM"
  ...
  hours_am_pmSleep(time) {
    ...
    if (hours < 12) {
      //return hours + ':' + min + ' AM';
      console.log(
        "here the sleep time mins change from 24 to ampm:   " +
          hours +
          ":" +
          min +
          " AM"
  ...
  // Old code with async function
  //  _hideNumPingsModal =async () =>{
  //     global.resetTimer();//global.globalTick=0;
  //    //  this.setState({idle:false});
  //     await this.handleBackAction(1);
  //     setTimeout(() => {
  //           this.setState({ numPingsModalShow: false,idle:false});
  //           }, 1000);
  //     }
  ...
  render() {
    let wakeDesc = "";
    let sleepDesc = "";
    let is24H = this.state.culture == "2";

    if (Platform.OS === "ios") {
      wakeDesc =
        resources.culture == "fr"
          ? this.state.waketime + " h"
          : this.state.waketime;
      sleepDesc =
        resources.culture == "fr"
          ? this.state.sleeptime + " h"
          : this.state.sleeptime;
    } else {
      if (is24H) {
            ...
            <ScrollView>
              <List.Section style={styles.mainStyle}>
                <List.Item
                  ...
                  right={() => (
                    <Switch
                      ...
                      accessible={true}
                      accessibilityLabel={
                        "Notifications" + this.state.notificationState
                      }
                      onValueChange={() => {
                        this.setState({
                          notificationState: !this.state.notificationState,
                        ...
                        if (this.state.notificationState) {
                          console.log(
                            "Switch OFF: Disabling Notifications, all notifications were removed"
                          );
                          Notifications.cancelAllScheduledNotificationsAsync();
                          global.sendouts = "";
                          AsyncStorage.setItem("Sendouts", sendouts);
                ...
                <View>
                  <List.Item
                    ...
                    title={
                      <ParsedText childrenProps={{ allowFontScaling: false }}>
                    ...
                    <View style={styles.centeredView}>
                      ...
                      >
                        <TimePickerPane
                          ...
                          onConfirm={this.onWakeConfirm.bind(this)}
                ...
                <List.Item
                  ...
                  left={() => (
                    <List.Icon
                      icon={require("../../assets/ic_wbc_language.png")}
                    />
                  )}
                ...
                <List.Item
                  ...
                  left={() => (
                    <List.Icon
                      icon={require("../../assets/ic_wbc_terms_condition.png")}
                    />
                  )}
                      ...
                      >
                        <View style={styles.radioButtonContainerStyle}>
                          <RadioButton.Android
                            value="1"
                            color="green"
                            uncheckedColor="#330033"
                          ></RadioButton.Android>
                          <Text style={styles.radioButtonTextEnStyle}>
                            English{" "}
                          </Text>
                        </View>
                        ...
                        <View style={styles.radioButtonContainerStyle}>
                          <RadioButton.Android
                            ...
                            uncheckedColor="#330033"
                          ></RadioButton.Android>
                          <Text style={styles.radioButtonTextFrStyle}>
                            Français
                          </Text>
                        </View>
                      </RadioButton.Group>
              ...
              <View>
                <Portal>
                  ...
                  >
                    <Dialog.Title>
                      {resources.getString("num_pings_dialog_title")}
                    </Dialog.Title>
                    <Dialog.Content>
                      <RadioButton.Group
                        onValueChange={(n) => {
